...
export interface ServerConfig {
  ...
  dashboard: boolean;
  /**
   * Play the recorded clips under `audioDir`. Off, every prompt is spoken by ConversationRelay's TTS
   * voice: one voice for the fixed text and the names and dates alike, and no seams between clips,
   * at the cost of the recorded voice.
   */
  clips: boolean;
}
...
export function loadConfig(env: Env): ServerConfig {
  ...
  const clipsSwitch = (env.CLIPS ?? 'on').toLowerCase();
