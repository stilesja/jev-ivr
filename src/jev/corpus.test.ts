...
describe('parseCorpus', () => {
  ...
  it('accepts timeOfDay on a scheduling opener, form or summary, and timePreference only at that summary', () => {
    ...
    const [opener] = parseCorpus('{"id":"t5","text":"book me with dr chen thursday afternoon","intent":"schedule_new","context":"no_form","timeOfDay":"afternoon"}');
    expect(opener?.timeOfDay).toBe('afternoon');
  ...
  it('rejects timeOfDay and timePreference off a scheduling context or opener, and a value outside their labels', () => {
    ...
    expect(() => parseCorpus('{"id":"x","text":"cancel my afternoon one","intent":"cancel","context":"no_form","timeOfDay":"afternoon"}')).toThrow(/timeOfDay needs a schedule_new or reschedule context/);
    expect(() => parseCorpus('{"id":"x","text":"what are your hours in the afternoon","intent":"none","context":"no_form","timeOfDay":"afternoon"}')).toThrow(/timeOfDay needs a schedule_new or reschedule context/);
