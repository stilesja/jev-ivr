...
- [ ] **Step 1: Session.** `Session` gains `frustratedTurns: number` (0) and `transferDeclined: boolean` (false), initialised in `newSession`, carried by `cloneSession`, and written to the trace record only if `summaryState`/`outcomeOf` copy the whole session (check; they do not need to). `PendingConfirmation` gains `| { target: 'transfer' }`. `buildTurnState` maps it to `{ target: 'transfer', value: 'connect you to a person' }` (widen the `TurnState.pendingConfirmation.target` union). Tests in `session.test.ts`/`state.test.ts`.

- [ ] **Step 2: Gate.** Replace gate 5 in `src/core/gates.ts`:
...
- [ ] **Step 3: Turn.** In `src/core/turn.ts`:
