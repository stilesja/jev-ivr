...
- Recording against the real model showed the three level descriptions did not mean what the corpus labels mean: a caller repeating a request with irritation scored `mild` 1.0 / `high` 0 ("I already told you, cancel it", "For the third time, Dr. Chen"), because the old `mild` listed "repeats a request with irritation" and the old `high` listed only swearing, insults, "this is ridiculous", and threats -- so five of the seven `fr-*` entries never reached `GATE_FRUSTRATION_HIGH` on the real model, and "Are you kidding me" only just did (`high` 0.51). The three descriptions were rewritten so that repeating oneself, counting attempts, and "are you kidding me" name `high`, and `mild` is a sigh or a "come on"/"seriously" on an otherwise plain request; the labels were left alone (`fr-03` and `fr-06` are still `mild` under the new wording). Separately, at the transfer offer "yes, connect me" trips the `wantsHuman` gate, which runs before the confirmation gate, so the handoff used to leave as `live-agent` with the generic line; that gate now hands off with reason `frustrated` when a `transfer` confirmation is pending, and the turn is still not counted as a frustrated one (a `handoff` verdict never carries a rung). The level rewrite re-keys the cassette in full: every recorded turn has to be re-recorded.
