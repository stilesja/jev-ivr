...
- **Candidates:** `candidateWordSpans(text)`: every 1- to 4-token n-gram that contains no digit or number word, does not start or end with a filler or function word (a small list kept with the span code: my, name, is, it's, this, the, a, an, and, um, uh, i, i'm, for, with, to, of, please, hi, hello, yes, no, calling), deduplicated in document order, capped at `MAX_WORD_SPANS` (320, separate from the number-span cap).
